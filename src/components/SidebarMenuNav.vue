    ...
    <ul class="
      ...
      ">
        <li class="group transition-all duration-200">
            <a href="/" :class="toggleSidebar ? 'flex flex-row items-start' : ''"
              ...
              relative
            ">
                    <span class="flex items-center"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                            fill="currentColor" viewBox="0 0 18 18" class="w-5 h-5">
                            <path d="M7.5 15.375v-4.5h3v4.5h3.75v-6h2.25L9 2.625l-7.5 6.75h2.25v6H7.5Z"></path>
                        </svg>
                    </span>
                </span>
        ...
        <li v-for="(item, index) in menuItems" :key="index" class="group transition-all duration-200">
            <div :class="toggleSidebar ? 'flex flex-row items-start' : ''" class="font-medium cursor-pointer relative"
            ...
            ">
                    ...
                    <span v-if="!toggleSidebar && item.new" class="
                absolute
                -top-1
                -right-4
                h-[14px]
              ...
              ">
                    ...
                    <span v-if="!toggleSidebar && item.count" class="
                absolute
                top-3
                -left-0.5
                w-1.5
              ...
              "></span>
                ...
                <Transition name="fade-in">
                    <span v-if="toggleSidebar && item.subItems.length > 0" class="flex items-center"><svg
                            xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            viewBox="0 0 16 16" class="w-6 h-6" :class="item.open
                                    ? 'absolute right-3 top-2 transform duration-200 transition'
                                    : 'absolute right-3 top-2 rotate-90 duration-200 transition'
                                ">
                            <path d="M4.94 5.53 8 8.583l3.06-3.053.94.94-4 4-4-4 .94-.94Z"></path>
                        </svg>
                    </span>
                </Transition>
            </div>
            <Transition name="height-slide-in" mode="out-in">
                <div v-if="item.subItems.length > 0 && (!toggleSidebar || item.open)" :class="`${!toggleSidebar
                    ...
                    <ul>
                        <template v-for="subItem in item.subItems">
                            <li v-if="subItem.state" :key="subItem.id" class="relative space-y-2" :class="toggleSidebar ? 'rounded py-[2.5] relative' : 'group/list'
                                ">
                                <div v-if="subItem.secondaryItems.length > 0" class="flex flex-1">
                          ...
                          transition-all
                          duration-300
                          hover:bg-[rgba(248,250,252,0.20)] hover:text-white
                        ">
                      ...
                      " :class="[
                        ...
                        ">
                                ...
                                <div v-if="subItem.secondaryItems.length > 0 &&
                                    (!toggleSidebar || subItem.secondary_open)
                                    " class="p-2" :class="!toggleSidebar
        ...
        ">
                                    <ul class="space-y-2">
                                        <li v-for="secondaryItem in subItem.secondaryItems" :key="secondaryItem.id">
                                            <a :href="secondaryItem.link" class="
                            flex
                            space-x-2
                            text-xsm/4
                            p-2
